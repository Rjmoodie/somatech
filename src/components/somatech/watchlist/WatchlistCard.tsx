    ...
    <Card className="relative hover-lift">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="min-w-0 flex-1">
            <h3 className="text-base md:text-lg font-semibold truncate">{item.ticker}</h3>
      ...
      <CardContent className="space-y-3 md:space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-lg md:text-2xl font-bold">${item.current_price.toFixed(2)}</span>
          <Badge className={`${getScenarioBadgeColor(item.dcf_scenario)} text-xs`}>
            {item.dcf_scenario.toUpperCase()}
          </Badge>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="font-medium text-sm md:text-base">${item.dcf_intrinsic_value.toFixed(2)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs md:text-sm text-muted-foreground">Upside/Downside</span>
        ...
        <div className="flex items-center justify-between pt-2 border-t">
          <span className={`font-medium text-xs md:text-sm ${getRecommendationColor(item.recommendation)}`}>
            {item.recommendation}
          </span>
