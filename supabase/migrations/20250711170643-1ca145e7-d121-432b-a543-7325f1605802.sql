...
ADD COLUMN IF NOT EXISTS category TEXT DEFAULT 'system' CHECK (category IN ('system', 'analysis', 'watchlist', 'portfolio', 'security', 'marketing')),
...
CHECK (type IN ('info', 'success', 'warning', 'error', 'alert', 'analysis_complete', 'watchlist_alert', 'price_target', 'weekly_summary'));
...
CREATE POLICY IF NOT EXISTS "Anyone can create feedback" 
ON public.user_feedback 
FOR INSERT 
WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Users can view all feedback" 
ON public.user_feedback 
FOR SELECT 
USING (true);
